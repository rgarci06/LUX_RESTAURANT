...
def _normalize_menu_item(row: dict) -> dict:
    return {
        "id": row.get("id"),
...
def _menu_supabase_client() -> Client:
    # Cliente nuevo para la carta (evita estado compartido raro entre peticiones).
...
def listar_menu():
    try:
        # Endpoint publico para pintar la carta en la web.
        menu_supabase = _menu_supabase_client()
        response = (
            menu_supabase
            .table(SUPABASE_MENU_TABLE)
            .select("*")
            .execute()
        )
        ...
        for category_key in categories:
            categories[category_key].sort(key=lambda item: (item.get("_order_index") or 0, str(item.get("name") or "").lower()))
...
def admin_editar_menu_item(item_id: str, payload: MenuItemUpdatePayload, authorization: str | None = Header(default=None)):
    try:
        # Solo admin puede editar platos.
        _require_admin(authorization)
        menu_supabase = _menu_supabase_client()
...
def admin_eliminar_menu_item(item_id: str, authorization: str | None = Header(default=None)):
    try:
        # Solo admin puede borrar platos.
        _require_admin(authorization)
        menu_supabase = _menu_supabase_client()
        respuesta = (
            menu_supabase
